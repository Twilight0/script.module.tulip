...
def parse_dom(html, name='', attrs=None, req=False, exclude_comments=False):
    ...
    if isinstance(html, unicode) or isinstance(html, DomMatch):
        ...
        try:
            ...
            log.log("HTML Decode Failed. Data length: %d" % len(html), log.LOGWARNING)
            try:
                html = [html.decode("utf-8", "replace")]
            except:
                log.log("HTML Decode Failed (Replace). Data length: %d" % len(html), log.LOGWARNING)
                html = [html]
    elif not isinstance(html, list):
        log.log("Input isn't list or string/unicode.", log.LOGWARNING)
        return ''

    if not name:
        log.log("Missing tag name", log.LOGWARNING)
        return ''

    if not isinstance(attrs, dict):
        log.log("Attrs must be dictionary", log.LOGWARNING)
        return ''
    ...
    # log.log("Done: %s" + (all_results), xbmc.LOGDEBUG)
    return all_results
